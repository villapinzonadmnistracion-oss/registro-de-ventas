...
fetchConfig().then(() => {
  ...
  if (!AIRTABLE_TOKEN || !BASE_ID || !CLIENTES_TABLE_ID || !VENTAS_TABLE_ID) {
  ...
  async function buscarCliente() {
    ...
    if (!rut) {
      mostrarAlerta("info", "⚠ Por favor, ingresa un RUT");
      return;
    }
  ...
  function agregarProducto() {
    ...
    const productoHTML = `
      <div class="producto-item">
        ...
        <div>
          <button class="btn btn-danger" onclick="eliminarProducto(this)" style="margin-top: 24px;">🗑</button>
        </div>
      </div>
    `;
  ...
  function eliminarProducto(btn) {
    ...
    if (items.length > 1) {
      ...
      mostrarAlerta("info", "⚠ Debe haber al menos un producto");
    }
  }
  ...
  async function registrarVenta() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        mostrarAlerta(
          "error",
          "❌ Error al registrar: " + (error.error?.message || "Error desconocido")
        );
      }
    } catch (error) {
      mostrarLoading(false);
